...
function invokeUrl(id, tag) {
    if (tag) {
        return `https://functions.yandexcloud.net/${id}?integration=raw&tag=${tag}`;
    } else {
...
class InvokeServiceImpl {
    ...
    async invoke(functionId, payload, tag) {
        ...
        const res = await fetch(invokeUrl(functionId, tag), opts);
